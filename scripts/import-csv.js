...
async function main() {
  console.log('=== 從 CSV 匯入美崙站數據 ===\n');

  try {
    // 1. 讀取 CSV
    console.log('1. 讀取 CSV 檔案...');
    const content = fs.readFileSync(csvPath, 'utf-8');
    const lines = content.trim().split('\n');
    const headers = lines[0].split(',').map(h => h.replace(/"/g, '').trim());
    console.log(`   欄位數: ${headers.length}`);
    console.log(`   數據行數: ${lines.length - 1}`);
      ...
      try {
        await prisma.$executeRaw`
          INSERT INTO customers_meilun (
            ...
            ${values[35] || ''}, ${values[36] || null}, ${values[37] || 0}, ${values[38] || 0},
            ...
            ${values[72] || null}, ${values[73] || 0}, ${values[74] || null},
            ${values[75] || null}, ${values[76] || ''}, ${values[77] || ''},
            ${values[78] || ''}, 1
          )
        `;
        inserted++;
        ...
        errors++;
        if (errors <= 3) {
          console.log(`   錯誤: ${e.message.substring(0, 60)}`);
        }
      }
    }

    console.log('\n✅ 完成!');
    console.log(`   CSV 總筆數: ${lines.length - 1}`);
