...
const config = {
  ...
  authentication: {
    ...
    options: {
      domain: '.',
      userName: process.env.USERNAME || 'tian7',
      password: ''
    }
  }
};
...
async function main() {
  console.log('=== 從 SQL Server 讀取美崙站數據 ===\n');

  try {
    // 連接 SQL Server - 使用 Windows 認證
    ...
    // 連接 PostgreSQL
    console.log('\n3. 連接 PostgreSQL...');
    console.log('   ✅ PostgreSQL 連接成功 (透過 Prisma)');
