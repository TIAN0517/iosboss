...
import { Mic, Volume2, Sparkles, StopCircle, VolumeX, Waves } from 'lucide-react'
...
export function VoiceQuickQuery({ onResult }: VoiceQuickQueryProps) {
  ...
  const [autoSpeak, setAutoSpeak] = useState(false) // 默認關閉自動朗讀
  ...
  const processAudioWithProfessionalASR = async (audioBlob: Blob) => {
    ...
    try {
      ...
      if (data.response) {
        ...
        // 如果有 TTS 音頻且啟用自動朗讀，播放它
        if (data.audio && data.audio.data && autoSpeak) {
  ...
  const speakText = (text: string) => {
    ...
    if ('speechSynthesis' in window) {
      ...
      const chineseVoice = voices.find(v => 
        v.lang.includes('zh') && (v.name.includes('Neural') || v.name.includes('Enhanced'))
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-easy-caption text-green-600 font-semibold">🤖 助手回覆：</p>
                  <button
                    onClick={() => {
                      triggerHaptic('light')
