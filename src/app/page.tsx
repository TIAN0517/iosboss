...
import { saveSession, loadSession, clearSession, isLoggedIn } from '@/lib/session-manager'
import {
  IOSCard,
  IOSCardHeader,
  IOSCardTitle,
...
export default function GasManagementSystem() {
  ...
  const [showImmersiveVoice, setShowImmersiveVoice] = useState(false)
    ...
    const content = (() => {
      switch (activeSection) {
        ...
        case 'chat':
          return (
            <>
              <ChatSection key={`chat-${refreshKey}`} />
              <SmartAIAssistant />
              <VoiceAIAssistant />
            </>
          )
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => {
          triggerHaptic('medium')
          setShowImmersiveVoice(true)
        }}
        className="fixed bottom-40 right-6 md:bottom-32 md:right-6 z-[100] flex flex-col items-center gap-2 group"
        title="語音助手"
      >
        <div className="relative">
          ...
          <div className="relative p-5 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 shadow-xl shadow-purple-500/40 hover:shadow-purple-500/60 hover:scale-110 active:scale-95 transition-all animate-pulse hover:animate-none">
            <Mic className="h-7 w-7 text-white" />
          </div>
        </div>
        <span className="bg-white px-3 py-1 rounded-full text-xs font-bold text-purple-600 shadow-lg whitespace-nowrap">
          語音助手
        </span>
      </button>
      ...
      {/* 沉浸式語音助手 */}
      {showImmersiveVoice && (
        <ImmersiveVoiceAssistant onClose={() => setShowImmersiveVoice(false)} />
      )}
    </div>
  )
}
