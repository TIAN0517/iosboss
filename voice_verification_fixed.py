...
def test_voice_functionality():
    ...
    for test_message in read_detection_cases:
        try:
            response = requests.post(
                "http://localhost:8889/api/voice/ai",
    ...
    print(f"   📊 检测结果:")
    print(f"      朗读模式: {read_count} 次")
    print(f"      AI对话模式: {ai_count} 次")
    print()
    ...
    for user_input, expected_context in context_tests:
        try:
            response = requests.post(
                "http://localhost:8889/api/voice/ai",
